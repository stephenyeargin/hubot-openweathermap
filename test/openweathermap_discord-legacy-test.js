...
// discord-legacy adapter uses parseVersion() returning '3.0.0' (falls back to text)
function applyDiscordLegacyVersion(bot) {
  bot.robot.parseVersion = () => '3.0.0';
}
...
describe('hubot-openweathermap discord-legacy no API key', () => {
  let bot;
  before(async () => {
    bot = await createTestBot(script, { adapterName: 'discord' });
    applyDiscordLegacyVersion(bot);
...
describe('hubot-openweathermap discord-legacy no default location', () => {
  let bot;
  before(async () => {
    bot = await createTestBot(script, { adapterName: 'discord' });
    applyDiscordLegacyVersion(bot);
