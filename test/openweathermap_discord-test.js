...
// discord adapter uses parseVersion() returning '11.0.0' (modern Discord embed path)
function applyDiscordVersion(bot) {
  bot.robot.parseVersion = () => '11.0.0';
}
...
describe('hubot-openweathermap discord', () => {
  describe('get weather for configured coordinates', () => {
    let bot;
    before(async () => {
      bot = await createTestBot(script, { adapterName: 'discord' });
      applyDiscordVersion(bot);
    ...
    it('responds with weather embed', () => {
      assert.ok(bot.sends.length >= 1);
      const msg = bot.sends[0];
      assert.ok(Array.isArray(msg.embeds));
      assert.equal(msg.embeds.length, 1);
      assert.equal(msg.embeds[0].data.title, 'Weather for Nashville-Davidson, Tennessee');
    });

    it('responds with alert embed', () => {
      assert.ok(bot.sends.length >= 2);
      const msg = bot.sends[1];
      assert.ok(Array.isArray(msg.embeds));
      assert.equal(msg.embeds.length, 1);
      assert.equal(msg.embeds[0].data.title, 'Flood Advisory');
    });
  });
  ...
  describe('get weather for a zip code', () => {
    ...
    it('responds with weather embed', () => {
      ...
      assert.ok(Array.isArray(msg.embeds));
      assert.equal(msg.embeds.length, 1);
      assert.equal(msg.embeds[0].data.title, 'Weather for Nashville, US');
    });
  ...
  describe('get weather for a city', () => {
    ...
    it('responds with weather embed', () => {
      ...
      assert.ok(Array.isArray(msg.embeds));
      assert.equal(msg.embeds.length, 1);
      assert.equal(msg.embeds[0].data.title, 'Weather for Denver, Colorado');
    });
    ...
    it('responds with alert embed', () => {
      ...
      assert.ok(Array.isArray(msg.embeds));
      assert.equal(msg.embeds.length, 1);
      assert.equal(msg.embeds[0].data.title, 'Red Flag Warning');
    });
  });
  ...
  describe('get weather for location outside US', () => {
    ...
    it('responds with weather embed', () => {
      ...
      assert.ok(Array.isArray(msg.embeds));
      assert.equal(msg.embeds.length, 1);
      assert.equal(msg.embeds[0].data.title, 'Weather for London, GB');
    });
...
describe('hubot-openweathermap discord no API key', () => {
  let bot;
  before(async () => {
    bot = await createTestBot(script, { adapterName: 'discord' });
...
describe('hubot-openweathermap discord no default location', () => {
  let bot;
  before(async () => {
    bot = await createTestBot(script, { adapterName: 'discord' });
    process.env.HUBOT_OPEN_WEATHER_MAP_API_KEY = 'abcdef';
